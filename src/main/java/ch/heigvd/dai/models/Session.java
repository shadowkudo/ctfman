...
public class Session {
   ...
   */
  public static @Nullable List<Session> findAllUserSessions(@NotNull String user) throws SQLException {
  ...
  public int insert() throws SQLException {
    ...
    String query = "INSERT INTO session (token, user_account, expires_at, ip_address, user_agent) VALUES (?,?,?,?,?)";
    try (Connection conn = DB.getConnection()) {
      PreparedStatement stmt = conn.prepareStatement(query);
      stmt.setString(1, token);
  ...
  /**
   * Delete a session using its token and user_account (prevents deleting a token
   * that might not be linked to the user)
   *
   * @TODO: Check if we should only require the token since the controller
